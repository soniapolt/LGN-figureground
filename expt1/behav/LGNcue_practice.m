function LGNcue_practice(subjNum,numTrials,feedback,trialLength,flicker)
% several practice trials to get observer comfortable  with trial structure.
% here, like in the quest, cue validity = 100%
% trials can be slowed down via the input parameters. 
% actual-experiment trialLength = 2, flicker = .2

% clear all;

input('Hit enter to proceed.');
params.cpdChange = 1.6; % 1.5 was used in our initial pilot
...
%%%% timing
params.trialLength = trialLength;       % in seconds
params.flicker = flicker;          % in seconds; also the target length
...
params.task.buffer = 1;                 % no targets in the first/last X flips
...
%%% assign bandpass filters - before trial starts
            params.stim.lowCPD = params.stim.cpd./params.cpdChange;%[0.3750 12];                                             % target increment - in scan, this was [3.25 11]
            params.stim.highCPD = params.stim.cpd*params.cpdChange; %[0.1667 5.3333];                                                 % target decrement
            fInc = params.stim.highCPD  * params.filtSize/params.ppd;
            fDec = params.stim.lowCPD  * params.filtSize/params.ppd;
...
for t = 1:length(trial)
    ...
    %             while 1
    %             [pressed, firstPress]= KbQueueCheck();
    %             if find(firstPress) == KbName('Space')
    %                 WaitSecs(1);
    %                 KbQueueFlush();
    %                 break
    %             end
...
eval(['save data/practice_s' num2str(subjNum) '_' expt.date '.mat params condition trial expt']);
